...
# Proteins are prepped with and without pQTL adjustment - pQTL adjustment used for elnets
# DNAm is prepped for 450k array

#############################################################################################################

### Prep GDF15 and Nt-pro-BNP test sets - without adjustments for pQTLs
...
### Prep GDF15 and Nt-pro-BNP levels - for elnets (with pQTL adjustment)
...
# We have the table of 3 SNPs for GDF15 levels 
...
#       n freq_geno        bJ     bJ_se           pJ      LD_r Protein
# 1 18414  0.754543 -0.465062 0.0110644  0.00000e+00 -0.247411   GDF15
# 2 18414  0.841079 -0.409408 0.0127872 6.33253e-225 -0.235338   GDF15
# 3 18414  0.779403 -0.138766 0.0114419  7.51309e-34  0.000000   GDF15


# We have the 3 SNPs extracted 

# > head(GDF_ex)
#          19_18500722_G_A 19_18501034_C_T 19_18501035_G_A
# 1 129146               1               1               0
# 2  95175               0               0               1
# 3 169894               0               0               0
# 4 100003               0               1               0
...
# # Residualise phenotypes together
# list <- c(3,5)
# for(i in list) {
#     d1[,i] <- scale(resid(lm(d1[,i] ~ age.x + factor(sex.x) + d1[,34] + d1[,35] + d1[,36] + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10
#          + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20, data=d1, na.action=na.exclude)))
# }

# # Get phenotype file scaled and write out for bayesR
# phen <- d1
# phenx <- scale(phen[,biomarker])
# write.table(x = t(as.matrix(as.numeric(phenx))), paste0("/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/02_BayesR/", name, "_170522_genetic_adjusted.csvphen"), sep = ",", row.names = F, col.names = F, quote = F) 
...
list <- c(4,6)
for(i in list) {
    W1[,i] <- scale(resid(lm(W1[,i] ~ age.x + factor(sex.x) + W1[,34] + W1[,35] + W1[,36] + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10
         + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20, data=W1, na.action=na.exclude)))
}

# list <- c(4,6)
# for(i in list) {
#     W4[,i] <- scale(resid(lm(W4[,i] ~ age.x + factor(sex.x) + W4[,34] + W4[,35] + W4[,36] + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10
#          + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20, data=W4, na.action=na.exclude)))
# }
...
W4 <- W4[c(29,4)]
write.csv(W4, "/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/W4_GDF15_pQTL.csv", row.names = F)

W1 <- W1[c(29,4)]
write.csv(W1, "/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/W1_GDF15_pQTL.csv", row.names = F)
...
# We have the table of 10 SNPs for ntprobnp levels 

# > table
#    Chr             SNP        bp refA     freq          b         se
# 1    1  1_11905981_A_G  11905981    A 0.846378  0.0148017 0.01321960
# 2    1  1_11908146_C_G  11908146    C 0.987356 -0.4447220 0.04712550
# 3    1  1_11919271_A_G  11919271    A 0.577163 -0.1980190 0.00978029
# 4    4   4_9209010_C_T   9209010    C 0.545908 -0.0553814 0.01469050
# 5    4  4_73458197_T_A  73458197    T 0.507910  0.0575759 0.01333290
# 6    4 4_103188709_C_T 103188709    C 0.918300 -0.1973130 0.01755780
# 7    8  8_22266530_C_T  22266530    C 0.698742  0.0608844 0.01040140
# 8   12 12_89829728_G_T  89829728    G 0.882861  0.0714746 0.01516240
# 9   12 12_89839072_C_T  89839072    C 0.769347  0.1389930 0.01135120
# 10  18 18_73681727_G_A  73681727    G 0.969302 -0.1698960 0.02974510


# We have the 10 SNPs extracted 

# > head(BNP_ex)
#          1_11905981_A_G 1_11908146_C_G 1_11919271_A_G 12_89829728_G_T
# 1 129146              0              0              1               0
# 2  95175              0              0              1               0
# 3 169894              0              0              1               0
# 4 100003              0              0              0               0
# 5 135861              1              0              1               1
# 6  84817              0              0              1               0
#   12_89839072_C_T 18_73681727_G_A 4_9209010_C_T 4_73458197_T_A 4_103188709_C_T
# 1               1               0             1              0               0
# 2               2               0             0              1               0
# 3               0               0             2              0               0
# 4               0               0             1              1               1
...
# list <- c(3,5)
# for(i in list) {
#     d1[,i] <- scale(resid(lm(d1[,i] ~ age.x + factor(sex.x) + d1[,34] + d1[,35] + d1[,36] + d1[,37] + d1[,38] + d1[,39] + d1[,40]
#       + d1[,41] + d1[,42] + d1[,43] + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10
#          + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20, data=d1, na.action=na.exclude)))
# }
...
# phen <- d1
# phenx <- scale(phen[,biomarker])
# write.table(x = t(as.matrix(as.numeric(phenx))), paste0("/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/02_BayesR/", name, "_16963_genetic_adjusted.csvphen"), sep = ",", row.names = F, col.names = F, quote = F) 
...
list <- c(4,6)
for(i in list) {
    W1[,i] <- scale(resid(lm(W1[,i] ~ age.x + factor(sex.x) + W1[,34] + W1[,35] + W1[,36] + W1[,37] + W1[,38] + W1[,39] + W1[,40]
      + W1[,41] + W1[,42] + W1[,43] + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10
         + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20, data=W1, na.action=na.exclude)))
}
...
# list <- c(4,6)
# for(i in list) {
#     W4[,i] <- scale(resid(lm(W4[,i] ~ age.x + factor(sex.x) + W4[,34] + W4[,35] + W4[,36] + W4[,37] + W4[,38] + W4[,39] + W4[,40]
#       + W4[,41] + W4[,42] + W4[,43] + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10
#          + PC11 + PC12 + PC13 + PC14 + PC15 + PC16 + PC17 + PC18 + PC19 + PC20, data=W4, na.action=na.exclude)))
# }
...
# Write out train and test proteins without pQTL adjustment, but with asjustment for residualisation and scaling/transforming
W4 <- W4[c(29,6)]
write.csv(W4, "/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/W4_BNP_pQTL.csv", row.names = F)

W1 <- W1[c(29,6)]
write.csv(W1, "/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/W1_BNP_pQTL.csv", row.names = F)



#############################################################################################################

### Prep GDF15 and Nt-pro-BNP levels - for elnets in 20k full GS (with pQTL adjustment)
...
# Write out train and test proteins with pQTL adjustment and with asjustment for residualisation and scaling/transforming
W1 <- W1[c(29,4)]
write.csv(W1, "/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/ALL_GDF15_pQTL.csv", row.names = F)
...
W1 <- W1[c(29,6)]
write.csv(W1, "/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/ALL_BNP_pQTL.csv", row.names = F)

#############################################################################################################

### Prep GDF15 and Nt-pro-BNP levels - for elnets in 20k full GS (without pQTL adjustment)
...
# # Subset to protein data and match IDs
# all_GDF <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/ALL_GDF15.csv")

# all_BNP <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/EpiScores_20k/GDFBNP/00_All_initial_files/02_Relatedness_mapping/ALL_BNP.csv")


# # Ensure sample order matches between x and y files for training and testing
# GDF_x <- W1[which(rownames(W1) %in% all_GDF$Sample_Sentrix_ID),]

# BNP_x <- W1[which(rownames(W1) %in% all_BNP$Sample_Sentrix_ID),]

# W1 = NULL

# GDF_x <- GDF_x[match(all_GDF$Sample_Sentrix_ID, rownames(GDF_x)),]
# identical(all_GDF$Sample_Sentrix_ID, rownames(GDF_x))

# BNP_x <- BNP_x[match(all_BNP$Sample_Sentrix_ID, rownames(BNP_x)),]
# identical(all_BNP$Sample_Sentrix_ID, rownames(BNP_x))

# # Save out train and test sets 
# library(data.table)
# saveRDS(GDF_x, "/Local_Scratch/Danni/GDFBNP/00_Preps/GDF_train_x_subset_EPIC.rds")
# saveRDS(BNP_x, "/Local_Scratch/Danni/GDFBNP/00_Preps/GDF_test_x_subset_EPIC.rds")
