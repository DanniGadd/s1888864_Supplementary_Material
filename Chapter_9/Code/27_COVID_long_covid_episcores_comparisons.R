...
# d1 <- read.csv("/Local_Scratch/Danni/GDFBNP/03_Cox/d1_W4_sample.csv")
...
# table(is.na(d2$GDF_score))
# table(is.na(d2$gdf15_rnk))

# table(is.na(d2$BNP_score))
# table(is.na(d2$nt.probnp_rnk))
...
# d2_GDF <- d2[complete.cases(d2$gdf15_rnk),]
# table(d2_GDF$S3_SymptomLengthAll)
...
# d2_BNP <- d2[complete.cases(d2$nt.probnp_rnk),]
# table(d2_BNP$S3_SymptomLengthAll)
