...
path <- '/Local_Scratch/Danni/GDFBNP/00_Updated_results/Results_LBC_brain/'
...
write.csv(basic, '/Local_Scratch/Danni/GDFBNP/00_Updated_results/Results_process_LBC_brain/basic.csv', row.names = F)
...
write.csv(full, '/Local_Scratch/Danni/GDFBNP/00_Updated_results/Results_process_LBC_brain/full.csv', row.names = F)
...
write.csv(full, '/Local_Scratch/Danni/GDFBNP/00_Updated_results/Results_process_LBC_brain/WBC.csv', row.names = F)
...
basic <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/GS_added_biomarkers/00_Analysis_20k/00_Updated_results/Results_process_LBC_brain/basic.csv")
full <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/GS_added_biomarkers/00_Analysis_20k/00_Updated_results/Results_process_LBC_brain/full.csv")
WBC <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/GS_added_biomarkers/00_Analysis_20k/00_Updated_results/Results_process_LBC_brain/WBC.csv")
...
#  [1] "SeqId.x"     "n.x"         "beta.x"      "SE.x"        "P.x"
#  [6] "ci.upper.x"  "ci.lower.x"  "cfi.x"       "rmsea.x"     "srmr.x"
# [11] "tli.x"       "phenotype.x" "FDR"         "retain"      "SeqId.y"
# [16] "n.y"         "beta.y"      "SE.y"        "P.y"         "ci.upper.y"
# [21] "ci.lower.y"  "cfi.y"       "rmsea.y"     "srmr.y"      "tli.y"
# [26] "phenotype.y" "assoc"       "SeqId"       "n"           "beta"
# [31] "SE"          "P"           "ci.upper"    "ci.lower"    "cfi"
# [36] "rmsea"       "srmr"        "tli"         "phenotype"

# write.csv(join, '/Local_Scratch/Danni/GDFBNP/00_Updated_results/Results_process_LBC_brain/joint.csv', row.names = F)
...
# join <- join[which(join$FDR < 0.05 & join$P.y < 0.05 | join$P < 0.05),]
# names(join) <- c('x', 'y', 'beta', 'SE', 'P', 'UCI', 'LCI')

bind$Protein <- c('GDF15 EpiScore', 'GDF15 EpiScore', 'GDF15 EpiScore')
bind$Phenotype <- c('General cognitive ability', 'Normal appearing white matter volume',
    'Total brain volume')
...
pdf("/Cluster_Filespace/Marioni_Group/Danni/GS_added_biomarkers/00_Analysis_20k/04_LBC/BRAIN_JOINT_RESULTS_PLOT.pdf", width = 20, height = 4)
