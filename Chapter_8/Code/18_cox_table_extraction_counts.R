...
# Write out predictor files in full 9537 for riccardo sensitivity analyses 
write.csv(pred, "/Cluster_Filespace/Marioni_Group/Danni/LBC_proteins_Jan2021/00_Running_pQTLs_regressed/predictors_file.csv", row.names = F)

# ## Outlier removal step 

# prot1 <- pred
# for(i in colnames(prot1)[2:111]){ 
  
#   cutoff1 = mean(prot1[,i], na.rm = T) + 4*sd(prot1[,i], na.rm = T)
#   cutoff2 = mean(prot1[,i], na.rm = T) - 4*sd(prot1[,i], na.rm = T)
  
#   prot1[,i][which(prot1[,i] > cutoff1 | prot1[,i] < cutoff2)] <- NA 
# } 
# pred <- prot1
...
# ADDITIONAL SENSITIVITY WITH RANK TRANSFORMATION
...
# write out copies for riccardo to use in sensitivity analyses 
write.csv(d1, "/Cluster_Filespace/Marioni_Group/Danni/LBC_proteins_Jan2021/00_Running_pQTLs_regressed/d1_rank_inverse.csv", row.names = F)
write.csv(d1, "/Cluster_Filespace/Marioni_Group/Danni/LBC_proteins_Jan2021/00_Running_pQTLs_regressed/d1_raw_episcores.csv", row.names = F)
...
Diabetes <- read.csv("/Cluster_Filespace/Marioni_Group/Danni/LBC_proteins_Jan2021/00_Running_pQTLs_regressed/00_Rerun/Cox/Input_tables/Run_by_Danni/Diabetes_combined.csv")
...
for(j in 1:length(clock)){ 
  ...
  cox$Event <- ifelse(cox$tte < -1, "NA", cox$Event) # add this in to change event to NA as well as tte
  ...
  output_hazard_AD[j,9] <-cox.zph(mod)[1][[1]][3]
      # save cases
    save_cases <- cox
    save_cases$time_to_event <- cox$tte
    cols <- save_cases
    #cols <- save_cases %>% select("Sample_Name", "Age", "Female", "dead", "aged", "Event", "age_death", "age_at_event", "time_to_event")
    write.csv(cols, file = paste0("/Cluster_Filespace/Marioni_Group/Danni/LBC_proteins_Jan2021/00_Running_pQTLs_regressed/00_Rerun/Cox/Extraction_counts_table/AD_all_cases.csv"), row.names = FALSE)
...
for(j in 1:length(clock)){
  for(i in 1:length(l)){ 
   tryCatch({ 
    ...
    cols <- save_cases
    #cols <- save_cases %>% select("Sample_Name", "Age", "Female", "dead", "aged", "Event", "age_death", "age_at_event", "time_to_event")
    write.csv(cols, file = paste0("/Cluster_Filespace/Marioni_Group/Danni/LBC_proteins_Jan2021/00_Running_pQTLs_regressed/00_Rerun/Cox/Extraction_counts_table/", names[[i]], "_all_cases.csv"), row.names = FALSE)
...
for(j in 1:length(clock)){ 
  for(i in 1:length(l.cancer)){ 
   tryCatch({ 
    ...
    cols <- save_cases
    #cols <- save_cases %>% select("Sample_Name", "Age", "Female", "dead", "aged", "Event", "age_death", "age_at_event", "time_to_event")
    write.csv(cols, file = paste0("/Cluster_Filespace/Marioni_Group/Danni/LBC_proteins_Jan2021/00_Running_pQTLs_regressed/00_Rerun/Cox/Extraction_counts_table/breast_all_cases_1.csv"), row.names = FALSE)
...
for(j in 1:length(clock)){ 
  ...
  output_hazard_IBD[j,9] <-cox.zph(mod)[1][[1]][3]
    ...
    cols <- save_cases
    #cols <- save_cases %>% select("Sample_Name", "Age", "Female", "dead", "aged", "Event", "age_death", "age_at_event", "time_to_event")
    write.csv(cols, file = paste0("/Cluster_Filespace/Marioni_Group/Danni/LBC_proteins_Jan2021/00_Running_pQTLs_regressed/00_Rerun/Cox/Extraction_counts_table/IBD_all_cases.csv"), row.names = FALSE)
